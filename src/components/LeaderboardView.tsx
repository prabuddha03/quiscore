...
interface LeaderboardViewProps {
  ...
  showParticipations?: boolean; // Show participation details
}
...
export function LeaderboardView({ 
      ...
      if (response.ok) {
        ...
        if (eventIds && eventIds.length > 0) {
          data = data.filter((entry: LeaderboardEntry) => 
            eventIds.some(eventId => entry.eventIds.includes(eventId))
          );
        }
  ...
  useEffect(() => {
    fetchLeaderboard();
  }, [eventIds, maxEntries, fetchLeaderboard]);
        ...
        {leaderboard.length === 0 ? (
          <div className="text-center text-gray-400 py-8">
            ...
            <p className="text-sm mt-2">
              {eventIds ? 'No participants found for the selected events.' : 'Create some leaderboards to get started.'}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {leaderboard.map((entry, index) => (
              ...
              >
                ...
                {showParticipations && entry.participations.length > 0 && (
